...
```{r}
fit <- lm(Freedom ~ gender, data = final_data)
  summary(fit)
...
```{r}
fit_1 <- lm(Freedom ~ Age, data = final_data)
  summary(fit_1)
```
...
```{r}
fit_2 <- lm(Freedom ~ Age + gender + Urban, data = final_data)
  summary(fit_2)
```
...
```{r}
fit_3 <- lm(Freedom ~ tribe, data = final_data)
  summary(fit_3)
```
...
```{r}
test_fit <- lm (Freedom ~ Urban + Age + tribe, data = final_data)
  summary(test_fit)
```
